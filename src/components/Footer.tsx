  ...
  return (
    <footer className="w-full bg-white py-20 px-6 border-t border-black/5 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-16 relative z-10">
        ...
        <div>
          <h4 className="font-black text-black mb-8 uppercase tracking-[0.3em] text-[10px]">Registry // Shop</h4>
          <ul className="space-y-4 text-xs font-bold text-black/40 uppercase tracking-widest">
            <li><Link href="/collections" className="hover:text-accent transition-colors">Sector: New</Link></li>
            <li><Link href="/collections" className="hover:text-accent transition-colors">Best Sellers</Link></li>
            <li><Link href="/collections" className="hover:text-accent transition-colors">All Assets</Link></li>
          </ul>
        </div>

        <div>
          <h4 className="font-black text-black mb-8 uppercase tracking-[0.3em] text-[10px]">Nodes // Support</h4>
          <ul className="space-y-4 text-xs font-bold text-black/40 uppercase tracking-widest">
            <li><Link href="/faq" className="hover:text-accent transition-colors">Query Grid</Link></li>
            <li><Link href="/shipping-returns" className="hover:text-accent transition-colors">Logistics</Link></li>
            <li><Link href="/contact" className="hover:text-accent transition-colors">Comms Sync</Link></li>
          </ul>
        </div>

        <div>
          <h4 className="font-black text-black mb-8 uppercase tracking-[0.3em] text-[10px]">Secure // Legal</h4>
          <ul className="space-y-4 text-xs font-bold text-black/40 uppercase tracking-widest">
            <li><Link href="#" className="hover:text-accent transition-colors">Privacy Protocol</Link></li>
            <li><Link href="#" className="hover:text-accent transition-colors">Terms of Sync</Link></li>
          </ul>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto mt-20 pt-8 border-t border-black/5 flex flex-col md:flex-row justify-between items-center gap-8 text-[10px] font-black uppercase tracking-[0.4em] text-black/20">
        ...
        <div className="flex gap-4">
          ...
          <div className="bg-black/5 px-3 py-1 rounded border border-black/5">CRYPTO</div>
        </div>
      </div>
    </footer>
  );
