  ...
  return (
    <>
      ...
      <button 
        ...
        className="fixed bottom-10 right-10 z-40 bg-black text-white p-6 rounded-3xl shadow-2xl hover:scale-110 active:scale-95 transition-all group border-none hover:shadow-[0_0_30px_rgba(244,143,177,0.3)]"
      >
        <ShoppingBag size={24} className="group-hover:rotate-12 transition-transform" />
        {totalItems > 0 && (
          <span className="absolute -top-2 -right-2 bg-accent text-white w-7 h-7 rounded-full flex items-center justify-center text-xs font-black border-4 border-white shadow-sm">
            {totalItems}
          </span>
        )}
      </button>
            ...
            >
              <div className="p-8 border-b border-black/5 flex items-center justify-between bg-white/50 backdrop-blur-md sticky top-0 z-10">
                <div className="space-y-1">
                  ...
                  <h2 className="text-3xl font-black uppercase tracking-tighter italic text-black">Your Cart</h2>
                </div>
              ...
              <div className="flex-1 overflow-y-auto p-8 space-y-8 scrollbar-hide">
                {items.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center text-center space-y-6 opacity-20">
                    ...
                    <p className="text-xl font-black uppercase tracking-widest italic">Your cart is empty.</p>
                  </div>
                ) : (
                  items.map((item) => (
                    <motion.div 
                    ...
                    >
                      <div className="w-24 h-24 rounded-2xl bg-pink-100/20 overflow-hidden glass border-black/5">
              ...
              {items.length > 0 && (
                <div className="p-8 border-t border-black/5 bg-pink-50/50 space-y-8">
                  ...
                  <Link 
                    href="/checkout" 
                    onClick={() => setIsOpen(false)}
                    className="w-full bg-black text-white py-6 rounded-2xl font-black text-lg uppercase tracking-tighter flex items-center justify-center gap-3 shadow-2xl hover:shadow-pink-500/20 hover:scale-[1.02] transition-all"
                  >
                    GO TO CHECKOUT
                    <ArrowRight size={20} />
                  </Link>
